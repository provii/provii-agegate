...
/**
 * Full machineServices.ts tests for 90%+ mutation coverage
...
describe("machineServices.ts - Full Coverage", () => {
  ...
  beforeEach(() => {
    ...
    // The redeem leg goes through fetchWithRetry (C1). Delegate it to the same
    // fetchWithTimeout mock so existing per-test response queues and call-index
    // assertions keep working unchanged (redeem records on the shared mock).
  ...
  describe("redeemChallenge rp-proxy mode", () => {
    it("uses rp-proxy URL when configured", async () => {
      ...
      // Redeem goes through fetchWithRetry (C1), which the beforeEach delegates
      // to the same fetchWithTimeout mock + queue used by the status poll.
  ...
  describe("redeemChallenge direct mode", () => {
    ...
    it("throws for other redeem errors (e.g. a 5xx that survived retries)", async () => {
      // The beforeEach delegates fetchWithRetry to the fetchWithTimeout mock,
      // so this single queued 5xx represents the final-attempt response after
      // retries are exhausted; redeem must still surface it as terminal.
      (fetchUtils.fetchWithTimeout as jest.Mock)
        .mockResolvedValueOnce(
          new Response(
            JSON.stringify({ status: "proof_ok_waiting_for_redeem" }),
