...
describe("qr.ts - Full Coverage", () => {
  ...
  describe("renderQrToCanvas", () => {
    ...
    describe("error handling and retry logic", () => {
      ...
      it("handles unexpected non-QRError exceptions", async () => {
        // Force an unexpected (non-QRError) error in the outer try block by
        // passing options with a getter that throws during qrOptions construction
        const badOptions = {
          get errorCorrectionLevel(): "L" | "M" | "Q" | "H" {
            throw new TypeError("Unexpected error in options access");
          },
        };

        await expect(
          renderQrToCanvas(mockCanvas, "test", badOptions),
        ).rejects.toThrow(QRError);

        const thrown = await renderQrToCanvas(mockCanvas, "test", badOptions)
          .then(() => {
            throw new Error("Expected rejection");
          })
          .catch((err: unknown) => err);
        expect(thrown).toBeInstanceOf(QRError);
        expect((thrown as QRError).code).toBe("RENDER_UNEXPECTED");
      });
    });
  });
});
